...
class TestReadyRunProtocol:
    ...
    def test_timeout_parsed_from_test_start(self):
        ...
        p.feed(_crc("PTR:READY"))
        p.command_sent()
        p.feed(_crc('PTR:TEST:START suite="HW" name="slow boot" timeout=120'))
    ...
    def test_timeout_zero_when_not_specified(self):
        ...
        p.feed(_crc("PTR:READY"))
        p.command_sent()
        p.feed(_crc('PTR:TEST:START suite="HW" name="fast test"'))
    ...
    def test_timeout_updates_per_test(self):
        ...
        p.feed(_crc("PTR:READY"))
        p.command_sent()
        p.feed(_crc('PTR:TEST:START suite="S" name="slow" timeout=60'))
        assert p.current_test_timeout == 60
        p.feed(_crc('PTR:TEST:START suite="S" name="fast" timeout=5'))
        assert p.current_test_timeout == 5
        p.feed(_crc('PTR:TEST:START suite="S" name="default"'))
        assert p.current_test_timeout == 0
    ...
    def test_timeout_reset_on_cycle_reset(self):
        ...
        p.feed(_crc("PTR:READY"))
        p.command_sent()
        p.feed(_crc('PTR:TEST:START suite="S" name="t" timeout=30'))
    ...
    def test_full_cycle_ready_run_done(self):
        ...
        p.feed(_crc('PTR:TEST:START suite="Suite" name="test_1" timeout=10'))
        p.feed("test line output")
        p.feed(_crc('PTR:TEST:START suite="Suite" name="test_2"'))
        assert p.state == ProtocolState.RUNNING
    ...
    def test_non_protocol_lines_dont_change_state(self):
        ...
        p.feed(_crc("PTR:DONE"))
        assert p.state == ProtocolState.FINISHED  # only PTR:DONE finishes
